...
function getTextDimension(input, defaultDimension) {
  ...
  var svgNode = _factories.hiddenSvgFactory.createInContainer(container);

  var textNode = _factories.textFactory.createInContainer(svgNode);
  ...
  var dimension = (0, _getBBoxCeil.default)(textNode, defaultDimension); // The nodes are added to the DOM briefly only to make getBBox works.
