 ...
 * under the License.
 */
var getInstance = (0, _utils.makeSingleton)(_TimeFormatterRegistry.default);
...
function getTimeRangeFormatter(formatId) {
  return new _TimeRangeFormatter.default({
    ...
    formatFunc: function formatFunc(range) {
      var format = getInstance().get(formatId);

      var _range$map = range.map(function (value) {
        return format(value);
      }),
          start = _range$map[0],
          end = _range$map[1];
...
function getTimeFormatter(formatId, granularity) {
  if (granularity) {
    var formatString = formatId || _TimeFormatsForGranularity.default[granularity];
    var timeRangeFormatter = getTimeRangeFormatter(formatString);
    return new _TimeFormatter.default({
      id: [formatString, granularity].join('/'),
      formatFunc: function formatFunc(value) {
        return timeRangeFormatter.format((0, _createTimeRangeFromGranularity.default)(value, granularity, timeRangeFormatter.useLocalTime));
      },
